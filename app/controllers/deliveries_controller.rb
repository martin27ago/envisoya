    ...
    if(!CiUY.validate(document) or image.nil?)
      ...
      begin
        if(@delivery = Delivery.create!(delivery_params))
          ...
          LoggerHelper.Log 'info', 'Cadete '+ @delivery.name + ' '+ @delivery.surname + ' creado con éxito.'

          redirect_to shippings_path
        else
          flash[:notice] = "Informacion invalida"
          LoggerHelper.Log 'error', 'No se pudo crear Cadete, error en los para.'
          render '/deliveries/new'
        end
      rescue
        flash[:notice] = "Informacion invalida"
    ...
    if(!CiUY.validate(document))
      ...
      begin
        ...
        if @delivery.update_attributes!(delivery_params)
          ...
          LoggerHelper.Log 'info', 'Cadete '+ @delivery.name + ' '+ @delivery.surname + ' actualizado con éxito.'
          redirect_to delivery_path(@delivery)
        else
          flash[:notice] = "Informacion inválida"
          LoggerHelper.Log 'error', 'Error al intentar editar cadete, parametros no válidos'
          redirect_to edit_delivery_path(current_delivery)
        end
      rescue
        flash[:notice] = "Informacion inválida"
